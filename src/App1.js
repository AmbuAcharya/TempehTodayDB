...
//   //     // const response = await axios.post(`${url}/uploadMFU?MFU_ID=${selectedMFUKey}&databaseKey=${selectedDatabaseKey}`,formData);
//   //     const response = await axios.post(`${url}/uploadMFU?MFU_ID=${selectedMFUKey}&databaseKey=${selectedDatabaseKey}`,formData, {
//   //       headers: {
//   //         'Content-Type': 'multipart/form-data',
//   //       },
...
//   //     if (response.ok) {
//   //       const responseData = await response.text(); // Since your server responds with a plain text
...
const FileUploader = () => {
  ...
  const handleFileUpload = async () => {
    try {
      ...
      // Create FormData
      const formData = new FormData();
      formData.append('file', selectedFile, selectedFile.name); // append file with a name
  
      const response = await axios.post('http://localhost:5000/tempehtoday-f866c/us-central1/app/uploadMFU12?MFU_ID=mfu_021&databaseKey=MFU_DB', formData, {
        headers: {
          'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        },
      });
      
      // const response = await axios.post(`http://localhost:5000/tempehtoday-f866c/us-central1/app/uploadMFU12?MFU_ID=mfu_021&databaseKey=MFU_DB`, {
      //   method: 'POST',
      //   body: formData,
      // });

      console.log("Response:", response.data); // Assuming the server sends a response message
