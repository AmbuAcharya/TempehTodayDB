...
app.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const result = { [userProvidedDocumentId]: {} };
    ...
    for (const sheetName of sheetNames) {
      ...
      excelData.forEach(row => {
        const { GB_ID, SB_ID, Date, Time, Soaking_start, Soaking_stop, Boiling_start, Boiling_reboil ,Boiling_stop, Inoculation_start, Inoculation_stop, } = row;

        if (!result[userProvidedDocumentId][GB_ID]) {
          result[userProvidedDocumentId][GB_ID] = {};
        }

        if (!result[userProvidedDocumentId][GB_ID][SB_ID]) {
          result[userProvidedDocumentId][GB_ID][SB_ID] = {
            BOILING: {
              Reboil: Boiling_reboil || null,
              STOP: Boiling_stop || null,
              STRT: Boiling_start || null,
            },
            DATE: Date || null,
            INOCULATION: {
    ...
    Object.keys(deviceData).forEach(gbID => {
      ...
      Object.keys(gbData).forEach(sbID => {
        ...
        const row = {
          ...
          DATE: sbData.DATE || '',
          TIME: sbData.TIME || '',
          BOILING_STRT: sbData.BOILING?.STRT || '',
